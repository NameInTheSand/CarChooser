...
fun ModelsScreen(
    ...
    when (screenState.value) {
        ...
        is ModelsScreenState.Content -> {
            ...
            var selectedItem by rememberSaveable(stateSaver = ModelSaver.modelSaver) {
...
private object ModelSaver {
    val modelSaver = Saver<ModelUiEntity?, Map<String, String>>(
        save = { manufacturer ->
            manufacturer?.let { mapOf("name" to it.name) }
        },
        restore = { map ->
            ModelUiEntity(map["name"] ?: "")
        }
    )
}
